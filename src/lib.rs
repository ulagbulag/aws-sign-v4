    ...
    &'a T: ::core::iter::IntoIterator<Item = (&'a String, &'a String)>,
{
    ...
    pub fn canonical_request(&'a self) -> String {
        ...
        format!(
            ...
            sha256 = digest::digest(&digest::SHA256, self.body)
                .encode_hex::<String>(),
        )
    }
...
pub fn signing_key(
    ...
    let region_key =
        ring::hmac::Key::new(ring::hmac::HMAC_SHA256, date_tag.as_ref());
    let region_tag =
        ring::hmac::sign(&region_key, region.to_string().as_bytes());

    let service_key =
        ring::hmac::Key::new(ring::hmac::HMAC_SHA256, region_tag.as_ref());
    let service_tag = ring::hmac::sign(&service_key, service.as_bytes());

    let signing_key =
        ring::hmac::Key::new(ring::hmac::HMAC_SHA256, service_tag.as_ref());
